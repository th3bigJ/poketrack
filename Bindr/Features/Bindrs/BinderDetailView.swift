...
struct BinderStylePickerSheet: View {
    ...
    private var formattedTotalValue: String {
        ...
        return formatter.string(from: NSNumber(value: 0)) ?? "$0.00"
    }
            ...
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    BinderStyleEditorSections(
                        pageLayout: Binding(
                            get: { binder.layout },
                            set: { binder.pageLayout = $0.rawValue }
                        ),
                        colour: $binder.colour,
                        texture: Binding(
                            get: { binder.textureKind },
                            set: { binder.texture = $0.rawValue }
                        ),
                        showPriceOverlay: $binder.showPriceOverlay,
                        showCardPreview: $binder.showCardPreview,
                        showValueOnCover: $binder.showValueOnCover,
                        titleTextColor: $binder.titleTextColor,
                        titleFontStyle: Binding(
                            get: { binder.titleFontStyleKind },
                            set: { binder.titleFontStyle = $0.rawValue }
                        ),
                        embossedCardID: $binder.embossedCardID,
                        embossedPokemonImageUrl: $binder.embossedPokemonImageUrl,
                        embossMode: Binding(
                            get: { binder.embossModeKind },
                            set: { binder.embossMode = $0.rawValue }
                        ),
                        embossedCandidateCardIDs: binder.slotList.map(\.cardID),
