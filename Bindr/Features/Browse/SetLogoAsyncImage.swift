...
/// Set logos from R2 — same `AsyncImage` pipeline as `BrowseView` card thumbnails (reliable WebP/PNG decode). Tries `AppConfiguration.setLogoURLCandidates` in order if a path 404s.
    ...
    var body: some View {
        Group {
            if trimmed.isEmpty || urls.isEmpty {
                Color.clear
            } else if candidateIndex < urls.count {
                AsyncImage(url: urls[candidateIndex]) { phase in
                    ...
                    case .success(let image):
                        image
                            ...
                            .scaledToFit()
                    case .failure:
                        // Try next candidate URL; if exhausted show nothing.
...
/// Set rarity / expansion symbol from `sets.json` `symbolSrc` — same candidate fallback as `SetLogoAsyncImage`.
