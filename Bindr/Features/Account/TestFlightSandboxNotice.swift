...
struct TestFlightSandboxNotice: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: BindrSpacing.sm) {
            ...
            Text("Subscribe and Restore use Apple's sandbox — no real charge. Sign in with a Sandbox Apple ID first:")
                .font(.system(size: 13))
                .foregroundStyle(.secondary)

            VStack(alignment: .leading, spacing: 6) {
                noticeStep(1, "Create a tester in App Store Connect → Users and Access → Sandbox.")
                noticeStep(2, "On this iPhone: Settings → App Store → Sandbox Account.")
                noticeStep(3, "Sign in with that Sandbox Apple ID, then return here to subscribe.")
            }
        }
