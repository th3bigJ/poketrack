...
extension CardPricingEntry {
    ...
    func tcgplayerMarketEstimateUSD() -> Double? {
        ...
        return num("marketPrice") ?? num("medianPrice") ?? num("lowestPrice")
    }
}
...
struct CardPriceHistory {
    ...
    func remappingVariantPlaceholder(_ placeholder: String, to displayVariant: String) -> CardPriceHistory {
        ...
        for (key, series) in self.series {
            let newKey: String
            if key.hasPrefix(placeholder + "/") {
                let rest = String(key.dropFirst(placeholder.count + 1))
                newKey = "\(displayVariant)/\(rest)"
            } else {
                newKey = key
            }
            newSeries[newKey] = series
        }
        return CardPriceHistory(series: newSeries)
    }
}
...
struct CardPriceTrends {
    ...
    /// See ``CardPriceHistory/remappingVariantPlaceholder(_:to:)``.
    func remappingVariantPlaceholder(_ placeholder: String, to displayVariant: String) -> CardPriceTrends {
        guard placeholder != displayVariant else { return self }
        var av = allVariants
        if let g = av.removeValue(forKey: placeholder) {
            av[displayVariant] = g
        }
        let newPrimary = (variant == placeholder) ? displayVariant : variant
        return CardPriceTrends(
            variant: newPrimary,
            grade: grade,
            change1d: change1d,
